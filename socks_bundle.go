...
//go:generate bundle -o socks_bundle.go -prefix socks golang.org/x/transport/internal/socks
...
func (code socksReply) String() string {
	...
	case socksStatusSucceeded:
		return "succeeded"
	case 0x01:
		return "general SOCKS interface failure"
	case 0x02:
...
// BoundAddr returns the address assigned by the proxy interface for
// connecting to the command target address from the proxy interface.
...
type socksDialer struct {
	...
	proxyNetwork string       // network between a proxy interface and a client
	proxyAddress string       // proxy interface address
...
// network.
//
// The returned error value may be a transport.OpError. When the Op field of
// transport.OpError contains "socks", the Source field contains a proxy
// interface address and the Addr field contains a command target
// address.
//
// See func Dial of the transport package of standard library for a
...
// DialWithConn initiates a connection from SOCKS interface to the target
// network and address using the connection c that is already
// connected to the SOCKS interface.
//
...
// proxy interface's network and address.
