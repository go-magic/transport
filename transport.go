...
// The high-level interface is in tcping_handler.go.
...
// for HTTPS URLs, depending on whether the interface supports HTTP/2,
// and how the Transport is configured. The DefaultTransport supports HTTP/2.
// To explicitly enable HTTP/2 on a transport, use golang.org/x/transport/http2
...
type Transport struct {
	...
	// then the transport dials using package transport.
	//
	...
	// are ignored. The returned transport.Conn is assumed to already be
	...
	// will only use the connection to the interface for a single
	// HTTP request.
	//
	...
	// time to wait for a interface's response headers after fully
	// writing the request (including its body, if any). This
	// time does not include the time to read the response body.
	ResponseHeaderTimeout time.Duration

	// ExpectContinueTimeout, if non-zero, specifies the amount of
	// time to wait for a interface's first response headers after fully
	// writing the request headers if the request has an
	// "Expect: 100-continue" header. Zero means no timeout and
	// causes the body to be sent immediately, without
	// waiting for the interface to approve.
	...
	// response bytes are allowed in the interface's response
	// header.
	//
...
// transport/http against an *http2.Transport that's either bundled into
// h2_bundle.go or supplied by the user via x/transport/http2.
//
...
// It must be called via t.nextProtoOnce.Request.
func (t *Transport) onceSetNextProtoDefaults() {
	...
	// golang.org/x/transport/http2 instead of the bundled copy, try to
...
func requiresHTTP1Transport(req *transportRequest) bool {
	return hasToken(req.Header.Get("Connection"), "upgrade") &&
		strings.EqualFold(req.Header.Get("Upgrade"), "websocket")
}
...
func (t *Transport) roundTrip(req *http.Request) (*http.Response, error) {
	...
	if isHTTP {
		for k, vv := range req.Header {
			if !httpguts.ValidHeaderFieldName(k) {
				...
				return nil, fmt.Errorf("transport/http: invalid header field name %q", k)
			}
			for _, v := range vv {
				if !httpguts.ValidHeaderFieldValue(v) {
					...
					return nil, fmt.Errorf("transport/http: invalid header field value %q for key %v", v, k)
				}
			}
		}
	}
	...
	if req.Method != "" && !validMethod(req.Method) {
		...
		return nil, fmt.Errorf("transport/http: invalid method %q", req.Method)
	}
	if req.URL.Host == "" {
		if req.Body != nil {
			req.Body.Close()
	...
	for {
		...
		// pre-CONNECTed to https interface. In any case, we'll be ready
		// to send it requests.
		...
		// Failed. cleanPool up and determine whether to retry.
		...
		if !pconn.shouldRetryRequest(req, err) {
			// Issue 16465: return underlying transport.Conn.Read error from peek,
...
func (pc *persistConn) shouldRetryRequest(req *http.Request, err error) bool {
	if http2isNoCachedConnError(err) {
		...
		// and violate the interface's max concurrent streams.
	...
	if !pc.isReused() {
		// This was a fresh connection. There's no reason the interface
		// should've hung up on us.
		//
		// Also, if we retried now, we could loop forever
		// creating new connections and retrying if the interface
	...
	if _, ok := err.(transportReadFromServerError); ok {
		// We got some non-EOF transport.Conn.Read failure reading
		// the 1st response byte from the interface.
		return true
	}
	if err == errServerClosedIdle {
		// The interface replied with io.EOF while we were trying to
...
func isReplayable(r *http.Request) bool {
	if r.Body == nil || r.Body == NoBody || r.GetBody != nil {
		...
		_, ke1 := r.Header["Idempotency-Key"]
		_, ke2 := r.Header["X-Idempotency-Key"]
		if ke1 || ke2 {
			return true
		}
	}
...
var ErrSkipAltProtocol = errors.New("transport/http: skip alternate protocol")
...
func (t *Transport) connectMethodForRequest(treq *transportRequest) (cm connectMethod, err error) {
	...
	cm.onlyH1 = requiresHTTP1Transport(treq)
	return cm, err
}
...
var (
	...
	// seen by a user if the interface shuts down an idle connection and sends its FIN
	// in flight with already-written POST body bytes from the client.
	// See https://github.com/golang/go/issues/19943#issuecomment-355607646
	errServerClosedIdle = errors.New("http: interface closed idle connection")
)

// transportReadFromServerError is used by Transport.readLoop when the
// 1 byte peek read fails and we're actually anticipating a response.
// Usually this is just due to the inherent keep-alive shut down race,
// where the interface closed the connection at the same time the client
// wrote. The underlying err field is usually io.EOF or some
// ECONNRESET sort of thing which varies by platform. But it might be
// the user's custom transport.Conn.Read error too, so we carry it along for
...
func (e transportReadFromServerError) Error() string {
	return fmt.Sprintf("transport/http: Transport failed to read from interface: %v", e.err)
}
	...
	if list, ok := t.idleConn[w.key]; ok {
		...
		for len(list) > 0 && !stop {
			...
			if tooOld {
				...
				// holding, and does a synchronous transport.Conn.Close.
...
func (t *Transport) dial(ctx context.Context, network, addr string) (net.Conn, error) {
	...
	if t.Dial != nil {
		...
		if c == nil && err == nil {
			err = errors.New("transport/http: Transport.Dial hook returned (nil, nil)")
		}
		return c, err
	}
...
func (w *wantConn) tryDeliver(pc *persistConn, err error) bool {
	...
	w.err = err
	if w.pc == nil && w.err == nil {
		panic("transport/http: internal error: misuse of tryDeliver")
	}
	close(w.ready)
	return true
}
...
func (t *Transport) customDialTLS(ctx context.Context, network, addr string) (conn net.Conn, err error) {
	...
	if conn == nil && err == nil {
		err = errors.New("transport/http: Transport.DialTLS or DialTLSContext returned (nil, nil)")
	}
	return
}
...
func (t *Transport) decConnsPerHost(key connectMethodKey) {
	...
	if n == 0 {
		...
		panic("transport/http: internal error: connCount underflow")
	}
...
func (t *Transport) getNetwork() string {
	if t.IPv6only {
		return "tcp6"
	}
	return "tcp4"
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod) (pconn *persistConn, err error) {
	...
	wrapErr := func(err error) error {
		if cm.proxyURL != nil {
			...
			return &net.OpError{Op: "proxyconnect", Net: t.getNetwork(), Err: err}
		}
		return err
	}
	if cm.scheme() == "https" && t.hasCustomTLSDialer() {
		var err error
		pconn.conn, err = t.customDialTLS(ctx, t.getNetwork(), cm.addr())
		if err != nil {
		...
		conn, err := t.dial(ctx, t.getNetwork(), cm.addr())
		if err != nil {
			return nil, wrapErr(err)
		}
		pconn.conn = conn
	...
	case cm.proxyURL == nil:
		// Request nothing. Not using a proxy.
	case cm.proxyURL.Scheme == "socks5":
		conn := pconn.conn
		d := socksNewDialer(t.getNetwork(), conn.RemoteAddr().String())
		if u := cm.proxyURL.User; u != nil {
		...
		if _, err := d.DialWithConn(ctx, conn, t.getNetwork(), cm.targetAddr); err != nil {
			conn.Close()
			return nil, err
		}
	...
	case cm.targetScheme == "https":
		...
		go func() {
			...
			// TLS interface will not speak until spoken to.
...
//	------------------------------    -------------------------
//	|http|foo.com                     http directly to interface, no proxy
//	|https|foo.com                    https directly to interface, no proxy
//	|https,h1|foo.com                 https directly to interface w/o HTTP/2, no proxy
...
func (pc *persistConn) mapRoundTripError(req *transportRequest, startBytesWritten int64, err error) error {
	...
	if pc.isBroken() {
		...
		return fmt.Errorf("transport/http: HTTP/1.x transport connection broken: %v", err)
	}
	return err
}
...
// closing a transport.Conn that is now owned by the caller.
...
func (pc *persistConn) readLoop() {
	...
	for alive {
		...
		if err != nil {
			if pc.readLimit <= 0 {
				err = fmt.Errorf("transport/http: interface response headers exceeded %d bytes; aborted", pc.maxHeaderResponseSize())
			}
...
// 100-continue") from the interface. It returns the final non-100 one.
// trace is optional.
	...
	for {
		...
		if is1xxNonTerminal {
			...
			if num1xx > max1xxResponses {
				return nil, errors.New("transport/http: too many 1xx informational responses")
			}
...
func write(r *http.Request, w io.Writer, usingProxy bool, extraHeaders http.Header, waitForContinue func() bool) (err error) {
	...
	//
	// cleanPool the host, in case it arrives with unexpected stuff in it.
	...
	if stringContainsCTLByte(ruri) {
		return errors.New("transport/http: can't write control character in Request.URL")
	}
	...
	if extraHeaders != nil {
		err = extraHeadersWrite(extraHeaders, w, trace)
		if err != nil {
			return err
		}
	}
...
func extraHeadersWrite(h http.Header, w io.Writer, trace *httptrace.ClientTrace) error {
...
// response from the interface. See comments in (*persistConn).wroteRequest.
...
func (pc *persistConn) wroteRequest() bool {
	...
	default:
		...
		// but the interface has already replied. In this case, we don't
...
// responseAndError is how the goroutine reading from an HTTP/1 interface
...
type requestAndChan struct {
	...
	// the interface responds 100 Continue, readLoop send a value
...
// concurrently waits on both the write response and the interface's
// reply.
...
var http2errRequestCanceled = errors.New("transport/http: request canceled")

var errTimeout error = &httpError{err: "transport/http: timeout awaiting response headers", timeout: true}

// errRequestCanceled is set to be identical to the one from h2 to facilitate
// testing.
var errRequestCanceled = http2errRequestCanceled
var errRequestCanceledConn = errors.New("transport/http: request canceled while waiting for connection") // TODO: unify?
...
func (pc *persistConn) roundTrip(req *transportRequest) (resp *http.Response, err error) {
	...
	// in case the interface decides to reply before reading our full
	// request body.
	startBytesWritten := pc.nwrite
	writeErrCh := make(chan error, 1)
...
func (tlsHandshakeTimeoutError) Error() string   { return "transport/http: TLS handshake timeout" }
